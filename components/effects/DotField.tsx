...
const DotField = memo(
  ({
    ...
    sparkle = false,
    waveAmplitude = 0,
    gradientFrom = 'rgba(209, 168, 147, 0.92)',
    gradientTo = 'rgba(29, 20, 16, 0.65)',
    glowColor = '#1d1410',
    backgroundImage,
    ...rest
  }: DotFieldProps) => {
    ...
    const glowGradientId = `dot-field-glow-${useId().replace(/:/g, '')}`;
        ...
        for (let i = 0; i < len; i++) {
          ...
          if (distSq < crSq && eng > 0.01) {
            ...
            if (isBulge) {
              ...
              const push = bulgeT * bulgeT * (p.bulgeStrength as number) * eng;
